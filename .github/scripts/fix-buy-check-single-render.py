...
controller_path = Path("src/clara-assistant-buy-check-tab.js")
overlay_path = Path("src/components/fresh/main-dashboard/assistant/ClaraAiEnvironmentOverlay.jsx")
registry_path = Path("src/runtime/installClaraRuntimePatches.js")
bridge_path = Path("src/clara-buy-check-auto-start-bridge.js")
...
controller = sub_once(
    controller,
    r"function startStaticBuyCheckFlow\(\) \{.*?\n\}\n\nfunction inferCategory",
...
controller = sub_once(
    controller,
    r"function renderBuyCheckBoard\(\) \{.*?\n\}\n\nfunction openBuyCheckMode\(\) \{.*?\n\}",
...
old_explicit = """  lastExplicitOpenRequestId = requestId || `buy-check-${Date.now()}`;
  buyCheckFlow = null;
  openBuyCheckMode();"""
new_explicit = """  lastExplicitOpenRequestId = requestId || `buy-check-${Date.now()}`;
  buyCheckFlow = createStaticBuyCheckFlow();
  openBuyCheckMode();"""
if old_explicit not in controller:
    raise RuntimeError("Explicit Buy Check initialization block not found")
controller = controller.replace(old_explicit, new_explicit, 1)

controller = sub_once(
    controller,
    r"\n    const startButton = event\.target\?\.closest\?\.\(\"\[data-clara-start-buy-check\]\"\);.*?\n    \}\n\n    const checkAgain",
    "\n    const checkAgain",
    "obsolete start button handler",
)
...
overlay = sub_once(
    overlay,
    r"function PauseEntryBoard\(\{ onClose \}\) \{.*?\n\}\n\nfunction MessageText",
...
checks = {
    controller_path: (
        ["function createStaticBuyCheckFlow()", 'board.dataset.claraBuyCheckOpeningBoard === "true"', 'data-clara-buy-check-active-question="true"'],
        ["Start Buy Check", "data-clara-start-buy-check"],
    ),
    overlay_path: (
        ['data-clara-buy-check-opening-board="true"', "Hi, Max! What do you want to buy?", 'placeholder="Type the item you want to buy"'],
        ["inputRef.current?.focus", "Your current Buy Check will open automatically."],
    ),
    registry_path: ([], ["clara-buy-check-auto-start-bridge"]),
}
...
for path, (required, forbidden) in checks.items():
    ...
    for marker in forbidden:
        if marker in text:
            raise RuntimeError(f"Forbidden {marker} remains in {path}")

print("Buy Check source-owned single render applied.")
