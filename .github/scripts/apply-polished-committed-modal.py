...
path = Path("src/components/fresh/main-dashboard/shell/DashboardPanelRenderer.jsx")
text = path.read_text()
...
old_actions = '''              <div className="mt-5 grid grid-cols-1 gap-2">
                <button
                  type="button"
...
new_actions = '''              <div className="mt-5 space-y-3">
                <button
                  type="button"
                  onClick={handleScheduleSession}
                  disabled={refreshing}
...
replace_once(old_actions, new_actions, "modal action area")
