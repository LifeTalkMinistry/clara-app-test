...
test("Manage Savings Goal uses a dedicated responsive action sheet", () => {
  assert.match(source, /const manageDialogClass = .*top-auto.*bottom-\[max\(0\.5rem,env\(safe-area-inset-bottom\)\)\].*translate-y-0/);
  assert.match(source, /sm:top-\[50%\].*sm:bottom-auto.*sm:translate-y-\[-50%\]/);
  assert.match(source, /<DialogContent className=\{manageDialogClass\}>/);
  assert.match(source, /pb-\[max\(1rem,env\(safe-area-inset-bottom\)\)\]/);
});

test("Savings detail and action dialogs never compete on screen", () => {
  assert.match(source, /const detailActionOpen = manageOpen \|\| addSavingsOpen \|\| useSavingsOpen \|\| overAmountOpen \|\| releaseSavingsOpen \|\| correctionOpen \|\| reconciliationOpen;/);
  assert.match(source, /open=\{Boolean\(goal\) && !detailActionOpen\}/);
  assert.match(source, /open=\{addSavingsOpen && !overAmountOpen\}/);
});
...
test("Manage action rows wrap and secondary dialogs open cleanly", () => {
  ...
  assert.match(manage, /items-start justify-start whitespace-normal/);
  assert.match(manage, /min-w-0 flex-1/);
  assert.match(manage, /whitespace-normal break-words/);
  ...
  assert.doesNotMatch(manage, /className=\{formDialogClass\}/);
});
`;
