...
const getSessionKey = (nextSession) =>
  nextSession?.access_token || nextSession?.user?.id || null;
...
export function AuthProvider({ children }) {
  ...
  const manualSignInSessionRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (
        event === 'SIGNED_IN' &&
        nextSessionKey &&
        manualSignInSessionRef.current === nextSessionKey
      ) {
        return;
      }
  ...
  const signIn = async ({ email, password }) => {
    ...
    manualSignInSessionRef.current = sessionKey;
    initializedRef.current = true;
    ...
    window.setTimeout(() => {
      applySession(nextSession, {
        ...
        .catch((backgroundError) => {
          console.error('background sign-in session apply error:', backgroundError);
          setLoading(false);
          setAuthReady(true);
        })
        .finally(() => {
          if (manualSignInSessionRef.current === sessionKey) {
