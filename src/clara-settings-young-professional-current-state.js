...
function installStyles() {
  ...
  style.textContent = `
    ...
    .clara-current-state-row,
    .clara-current-state-card,
    .clara-current-state-instruction {
      border: 1px solid rgba(165, 243, 252, 0.14);
      background:
        radial-gradient(circle at 0% 0%, rgba(34, 211, 238, 0.085), transparent 38%),
        radial-gradient(circle at 100% 100%, rgba(124, 58, 237, 0.065), transparent 42%),
    ...
    .clara-current-state-row:hover {
      border-color: rgba(165, 243, 252, 0.2);
      background:
        radial-gradient(circle at 0% 0%, rgba(34, 211, 238, 0.11), transparent 38%),
        radial-gradient(circle at 100% 100%, rgba(124, 58, 237, 0.09), transparent 42%),
        rgba(255, 255, 255, 0.055);
    }

    .clara-current-state-icon {
      display: flex;
      height: 2.75rem;
      width: 2.75rem;
      flex-shrink: 0;
    ...
    .clara-current-state-icon svg,
    .clara-current-state-row > svg,
    .clara-current-state-back svg {
      height: 1rem;
      width: 1rem;
    }
    ...
    .clara-current-state-text {
      min-width: 0;
      flex: 1;
    }

    .clara-current-state-text p {
      overflow: hidden;
    ...
    .clara-current-state-instruction {
      ...
      background:
        ...
        linear-gradient(135deg, rgba(6, 78, 59, 0.26), rgba(15, 23, 42, 0.44));
      padding: 1.15rem 1.1rem;
    }

    .clara-current-state-kicker {
      display: inline-flex;
    ...
    .clara-current-state-instruction h2 {
      margin: 0;
      font-size: 1rem;
      line-height: 1.25;
      font-weight: 950;
    ...
    .clara-current-state-instruction p {
      margin-top: 0.55rem;
      max-width: 34ch;
    ...
    .clara-current-state-card:hover:not(:disabled) {
    ...
    .clara-current-state-card:disabled {
      opacity: 0.68;
    }

    .clara-current-state-card h3 {
      margin: 0;
    ...
    .clara-current-state-card p {
      margin-top: 0.45rem;
      color: rgba(236,253,255,0.50);
      font-size: 0.74rem;
      font-weight: 650;
      line-height: 1.55;
    }

    .clara-current-state-card small {
      display: inline-flex;
      margin-top: 0.7rem;
      border-radius: 999px;
      border: 1px solid rgba(165,243,252,0.14);
...
async function loadYoungProfessional(button) {
  const previousText = button?.querySelector("small")?.textContent || "Activate current state";

  try {
    if (button) {
      button.disabled = true;
      const small = button.querySelector("small");
      if (small) small.textContent = "Loading current state...";
    }

    setStatus("Preparing Young Professional as your current financial setup...", "success");
    const result = await activateYoungProfessionalCurrentState();
    setStatus("Young Professional is now active as the current financial setup. CLARA will refresh the dashboard using this data.", "success");
    ...
    <div class="space-y-2.5">
      <button type="button" class="clara-current-state-row">
        ...
        <div class="clara-current-state-text">
          <p>Explore CLARA</p>
          <span>Try a real current-state setup for learning</span>
        </div>
...
function createPage() {
  ...
  page.innerHTML = `
    ...
    <div class="clara-current-state-instruction">
      ...
      <h2>Choose current financial setup</h2>
      <p>This will write the selected life-stage setup into the current user's local financial records, so Dashboard and CLARA read it as active data.</p>
    </div>
    ...
    <button type="button" class="clara-current-state-card">
      ...
      <p>Stable salary, independence pressure, payday confidence, small lifestyle leaks, and early savings discipline.</p>
      <small>₱32,000 · Two payroll cutoffs · Current-state data</small>
    </button>
  `;
