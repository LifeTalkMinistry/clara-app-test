...
function slideNineLeakCategory(slideThree, slideFour) {
...
function slideNineLeakAmount(slideFour, slideFive, slideNine) {
  const candidates = [
...
function actionDifficulty(leakAmount = 0) {
  if (leakAmount <= 0) return "Manageable";
  const records = snapshotRecords();
  ...
  return totalExpenses > 0 && leakAmount / totalExpenses >= 0.3 ? "Needs discipline" : "Manageable";
}

function slideNineBody(goodHabits, category, leakAmount) {
  const hasEnoughData = goodHabits !== NEEDS || Boolean(category) || leakAmount > 0;
...
function finalizeSlideNine() {
  ...
  const category = slideNineLeakCategory(slideThree, slideFour);
  const leakAmount = slideNineLeakAmount(slideFour, slideFive, slide);
  const recover = leakAmount > 0 ? `Recover ${money(leakAmount)}` : "Build more history";
  const goodHabits = goodHabitsToProtect();

  setHero(slide, leakAmount > 0 ? recover : "No major leak to fix");
  setRows(slide, [
    ...
    ["Action Difficulty", actionDifficulty(leakAmount)],
    ["Next Step", category ? `Set ${category} limit` : "Track 7 days"],
  ]);

  const body = slide.querySelector(".clara-forecast-report-body");
  if (body) body.textContent = slideNineBody(goodHabits, category, leakAmount);
