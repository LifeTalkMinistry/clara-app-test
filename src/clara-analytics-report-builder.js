...
export function buildClaraAnalyticsReport(snapshot = {}, options = {}) {
  ...
  const avgIncome = income / Math.max(analysisMonths, 1);
  ...
  const cards = [
    ...
    { eyebrow: "03 / INCOME REALITY", title: "Income Reality", tone: "neutral", hero: safe(money(income)), body: "This shows the actual income CLARA found inside the selected analysis window.", stats: [stat("Actual Income", safe(money(income))), stat("Average Monthly Income", safe(money(avgIncome))), stat("Income Records Used", safe(`${incomeRecords.length || txIncomeRecords.length} record${(incomeRecords.length || txIncomeRecords.length) === 1 ? "" : "s"}`)), stat("Income Source Count", safe(`${records.incomeSources.length} source${records.incomeSources.length === 1 ? "" : "s"}`))] },
