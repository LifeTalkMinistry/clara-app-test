...
const walletId = (wallet) => String(wallet?.id || wallet?.wallet_id || wallet?.walletId || "").trim();
...
export default function SavingsGoalsIntegrated() {
  ...
  const handleAddSavings = async (goal, inputAmount, sourceWalletId, forcedAmount = null) => {
    ...
    const updatedGoal = normalizeGoal({
      ...
      savingsActivityLog: buildActivity(goal, {
        ...
        storageWalletName: savedInWallet.name,
        storage_wallet_name: savedInWallet.name,
        note: walletId(sourceWallet) === walletId(savedInWallet) ? `Protected in ${savedInWallet.name}` : `Moved from ${sourceWallet.name} to ${savedInWallet.name}`,
        createdAt: now,
        created_at: now,
      }),
      updated_date: now,
  ...
  const handleUseSavings = async (goal, amount, reason) => {
    ...
    const updatedGoal = normalizeGoal({
      ...goal,
      saved_amount: Math.max(toNumber(goal.saved_amount) - safeAmount, 0),
      savingsActivityLog: buildActivity(goal, {
