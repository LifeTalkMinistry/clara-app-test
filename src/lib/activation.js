...
export async function validateActivationCode({ code, user, plan }) {
  ...
  if (!cleanCode) throw new Error("Enter your activation code.");
  if (!user?.id) throw new Error("Sign in before activating CLARA.");
  ...
  if (codePlan !== planKey) throw new Error("This code is for a different CLARA tier.");
  if (data.used_at || data.status === "used") throw new Error("This activation code has already been used.");
