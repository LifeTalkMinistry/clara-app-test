...
function buildContextSignals({ assessment, schedule, memory, emergencyFund, savingsGoals }) {
  ...
  const obligation = schedule
    .map((event) => ({ event, amount: amountFromRecord(event), date: dateFromRecord(event) }))
  ...
  const spendableAfter = toNumber(assessment.spendableAfter);
  const upcomingObligationRisk = obligation && spendableAfter < obligation.amount ? "critical" : obligation ? "present" : "none";
...
function buildContextPackage(flowValue, contextValue) {
  ...
  return {
    ...
    finance: {
      spendableWallets: wallet.eligibleFundingWallets.map((entry) => ({
        id: entry.id,
        name: entry.name,
        grossBalance: entry.grossBalance,
        reservedBalance: entry.reservedBalance,
        spendableBalance: entry.spendableBalance,
      })),
      protectedWallets: wallet.wallets.filter((entry) => entry.protected).map((entry) => ({
        id: entry.id,
        name: entry.name,
        balance: entry.grossBalance,
