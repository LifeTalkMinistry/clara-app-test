...
function resolveEndpoint() {
  ...
  return isNativeLike || isGitHubPages
    ? MASTERCLASS_PRODUCTION_ENDPOINT
    : MASTERCLASS_ENDPOINT;
}

function makeError(payload = {}, status = 0) {
  const error = new Error(
    payload?.error || payload?.message || "CLARA could not generate that clarification right now."
  );
...
export async function requestBudgetMasterclassAi({ mode, prompt, signal } = {}) {
  ...
  if (!["explain_another_way", "follow_up_question"].includes(cleanMode)) {
    throw makeError({ error: "That CLARA masterclass clarification mode is not supported." }, 400);
  }
  ...
  try {
    const response = await fetch(resolveEndpoint(), {
      ...
      body: JSON.stringify({ mode: cleanMode, prompt: cleanPrompt }),
    });
    ...
    return { text, usage: payload?.usage || null, model: payload?.model || "" };
  } catch (error) {
    if (error?.name === "AbortError") {
      throw makeError({
        code: "CLARA_MASTERCLASS_AI_TIMEOUT",
        error: "That clarification took too long. You can try again or continue the masterclass.",
      }, 504);
    }
    throw error;
  } finally {
