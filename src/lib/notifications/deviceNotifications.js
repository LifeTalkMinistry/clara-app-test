...
function formatDiagnosticError(error) {
  ...
  if (/Firebase service account|Firebase credentials|FCM/i.test(message)) {
    return new Error(
      "CLARA push delivery is not fully configured on the backend. Check the Firebase/FCM server credentials."
    );
  }
...
export async function syncExpenseLogLocalNotifications({
  ...
  if (permission !== "granted") {
    if (permission === "denied") {
      console.warn(
        "CLARA expense log local notifications skipped because Android notification permission is denied."
      );
    }
...
export async function sendRealPushTestNotification() {
  ...
  try {
    if (!getStoredBackendToken()) {
      throw new Error("Sign in to your CLARA account before testing push notifications.");
    }
    ...
    } else if (environment.preferredChannel === "native_push") {
      const enableResult = await enableNativePushNotifications({ userId: "current" });
      if (enableResult.permission === "denied") {
        throw new Error(
          "Phone notification permission is denied. Enable CLARA notifications in your device settings, then try again."
        );
      }
      if (!enableResult.configured || !enableResult.token) {
        throw new Error(
          "CLARA could not register this phone for push notifications. Check the Firebase/FCM device configuration."
        );
      }
    } else {
    ...
    if (!Number(data?.sent || data?.nativeSent || data?.webSent || 0)) {
      throw new Error(
        data?.reason || "CLARA backend did not report a successful push delivery."
      );
    }
