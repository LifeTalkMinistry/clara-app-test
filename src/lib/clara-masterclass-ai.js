...
function resolveEndpoint() {
  ...
  const isNativeLike = protocol === "capacitor:" || protocol === "ionic:" || protocol === "file:" || hostname === "localhost" || hostname === "127.0.0.1" || Boolean(window.Capacitor?.isNativePlatform?.());
...
function makeError(payload = {}, status = 0) {
  ...
  error.code = payload?.code || "CLARA_MASTERCLASS_AI_FAILED"; error.status = status; error.usage = payload?.usage || null; return error;
}
...
export async function requestClaraMasterclassAi({ masterclassId, mode, prompt, signal } = {}) {
  ...
  if (!token) throw makeError({ code: "CLARA_MASTERCLASS_AUTH_REQUIRED", error: "Your CLARA session is required before I can generate a custom clarification." }, 401);
  ...
  try {
    const response = await fetch(resolveEndpoint(), {
      ...
      body: JSON.stringify({
        masterclassId: cleanMasterclassId,
        mode: cleanMode,
        prompt: `MODE: ${cleanMode.toUpperCase()}\n${cleanPrompt}`,
      }),
    });
    ...
    if (error?.name === "AbortError") throw makeError({ code: "CLARA_MASTERCLASS_AI_TIMEOUT", error: "That clarification took too long. You can try again or continue the masterclass." }, 504);
    throw error;
  } finally {
