...
function confirmationText(flow) {
  ...
  if (flow.planningStatus === "planned" && flow.budgetCoverage) {
    ...
    return `This is covered by your ${flow.budgetCoverage.budgetTitle}${flexibleText} budget. You’re considering ${flow.item} for ${money(flow.price)}. The budget has ${money(flow.budgetCoverage.remaining)} available and would have ${money(flow.budgetCoverage.remainingAfter)} left. Did I get that right before I run the full Buy Check?`;
  }
  if (assessment?.status === "partial" && assessment.selectedBudget) {
    return `You’re considering ${flow.item} for ${money(flow.price)} because ${flow.reason}. Your ${assessment.selectedBudget.title} budget only has ${money(assessment.selectedBudget.remaining)} left, leaving a ${money(assessment.shortfall)} shortfall. Did I get that right before I run the full Buy Check?`;
  }
