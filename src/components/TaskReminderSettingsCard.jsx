        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
          ...
          <div className="mt-3 rounded-2xl border border-white/10 bg-black/20 p-3">
            ...
            <p className="mt-1 text-xs leading-6 text-white/52">
              {pushSupported
                ? "Installed PWAs use CLARA Web Push; native Android can use its native push channel. Each enabled device is registered separately."
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            >
              {localTestSending ? "Sending local test..." : "Send local device test"}
            </button>
            <button
              type="button"
              onClick={onSendRealPushTest}
              disabled={realPushTestSending}
            ...
            >
              {realPushTestSending ? "Sending real push..." : "Send real push test"}
            </button>
          </div>
          ...
          <div className="mt-3 grid gap-2 text-[11px] leading-5 text-white/45">
            <p className="rounded-2xl border border-white/10 bg-black/20 p-3">
              Local test = generated by this device. It verifies permission and local notification display only.
            </p>
            <p className="rounded-2xl border border-cyan-300/10 bg-cyan-300/[0.06] p-3 text-cyan-50/60">
              Real push test = sent from CLARA’s server-side notification channel. On an installed PWA this uses Web Push and can arrive while CLARA is closed.
            </p>
          </div>
        </div>
      </div>
