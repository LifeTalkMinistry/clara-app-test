...
export default function CommunityGuideTour({ open, onClose }) {
  ...
  const placePanelAtTop = useMemo(() => {
    if (!targetRect || typeof window === "undefined") return false;
    const targetCenter = targetRect.top + targetRect.height / 2;
    return targetIsLarge || targetCenter > window.innerHeight * 0.58;
  }, [targetIsLarge, targetRect]);
  ...
  return (
    <div className="fixed inset-0 z-[2147482500]" aria-hidden={false}>
      ...
      <section
        ...
        className={`absolute left-1/2 w-[min(calc(100vw-34px),360px)] -translate-x-1/2 overflow-hidden rounded-[24px] border border-white/[0.10] bg-[linear-gradient(180deg,rgba(8,20,38,0.985),rgba(6,14,29,0.99))] text-white shadow-[0_24px_70px_rgba(0,0,0,0.58),0_0_30px_rgba(34,211,238,0.08),inset_0_1px_0_rgba(255,255,255,0.055)] backdrop-blur-2xl ${
          placePanelAtTop
