export default function DashboardTopNav({
  ...
  return (
    <div
      className={`relative shrink-0 ${elevated ? "z-[100]" : "z-30"} ${dashboardScale.headerOuter}`}
      data-dashboard-top-nav="true"
    >
        ...
        >
          ...
          <div className="relative grid grid-cols-4 gap-1.5 sm:gap-2">
            {items.map((item, index) => {
              ...
              const isActive = item.forceActive || activeKey === item.key;
                ...
                >
                  <div
                    className={`relative flex w-full flex-col items-center justify-center overflow-hidden border transition duration-200 ${
                      isDisabled ? "" : "hover:-translate-y-[1px] active:scale-[0.985]"
                  ...
                  >
                    {isActive || isHighlighted ? (
                      ...
                      <div
                        className={`pointer-events-none absolute inset-0 rounded-[inherit] opacity-0 transition duration-200 group-hover:opacity-100 ${
                          themeIsLight
                            ? "bg-[radial-gradient(circle_at_top,rgba(148,163,184,0.14),transparent_58%)]"
                            : "bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.08),transparent_58%)]"
                        }`}
                      />
                    )}
                  ...
                  {index < items.length - 1 ? (
                    <div
                      className={`pointer-events-none absolute -right-1 top-1/2 hidden h-9 w-px -translate-y-1/2 bg-gradient-to-b from-transparent ${themeDividerClass} to-transparent sm:block`}
                    />
                  ) : null}
                </button>
              );
