...
function formatForecastValue(value) {
  ...
  if (typeof value === "object") {
    ...
    return hasValue(direct) ? String(direct) : "Available";
  }
  return String(value);
}
...
function buildForecastPhaseOneSnapshot(context = {}) {
  ...
  const transactions = mergeUniqueArrays(walletTransactions, firstArray(source, ["transactions", "recentTransactions", "finance.transactions"]));
  ...
  const emergencyFund = financeSnapshot.emergencyFund || rawEmergencyFund || {};
  ...
  const recentExpenseActivity = expenses.length || transactions.length ? `${expenses.length + transactions.length} visible record(s)` : firstValue(source, ["recentExpenseActivity", "recentBehavior.recentExpenseActivity", "behavioralObservation.recentExpenseActivity", "ai_financial_memory.recentExpenseActivity"]);
  ...
  const currentMoney = { walletCount: wallets.length, totalWalletBalance, moneyLeft, safeSpendableMoney, emergencyProtectedAmount };
  const moneyComingIn = { incomeRecordsCount, incomeSourcesCount: incomeSources.length, expectedIncome, paydayTiming };
  const moneyGoingOut = { expensesCount: expenses.length, transactionsCount: transactions.length, transfersCount: transfers.length, recurringExpensesCount: recurringExpenses.length, subscriptionsCount: subscriptions.length, billsCount: bills.length, debtObligationsCount: debtObligations.length };
  const budgetPressure = { budgetsCount: budgets.length || (financeSnapshot.hasActiveBudgetPlan ? 1 : 0), budgetCategoriesCount: budgetCategories.length, budgetRemaining, plannedExpensesCount, unplannedExpensesCount, outsidePlanSpendingCount };
  const savingsPressure = { savingsGoalsCount: savingsGoals.length, savingsProgress: financeSnapshot.savingsProgress, emergencyFund: emergencyFundSaved, goalDeadlinesCount: countGoalDeadlines(savingsGoals) };
  const behaviorRisk = { recentExpenseActivity, stressSpendingHabit, commonImpulsePurchases, biggestSpendingWeakness, unplannedSpendingCount: unplannedExpensesCount };
  const schedulePaydayRisk = { paydayCycle: paydayTiming, workSchedule, sleepPattern, energyDrop, burnoutIndicators };
...
function isForecastActionTarget(target) {
  ...
  return text.includes("future money forecast") || (text.includes("forecast") && text.includes("predict where your money is heading")) || text === "forecast";
}
...
export default function ClaraAiEnvironmentOverlay(props) {
    ...
    const handleForecastClick = (event) => {
      ...
      forecastTimerRef.current = window.setTimeout(() => {
        try {
          ...
          setForecastMessages([userMessage, { ...loadingMessage, text: forecastReadyText(snapshot), source: "forecast_phase_one" }]);
        } catch (error) {
          console.error("[CLARA Forecast Phase 1] Snapshot failed", error);
          setForecastMessages([userMessage, { ...loadingMessage, text: "Forecast snapshot could not be prepared right now. CLARA stayed safe and did not change your records.", source: "forecast_phase_one" }]);
        } finally {
          forecastTimerRef.current = null;
        }
      }, 650);
  ...
  const mergedMessages = useMemo(() => [...asArray(messages), ...forecastMessages], [messages, forecastMessages]);
