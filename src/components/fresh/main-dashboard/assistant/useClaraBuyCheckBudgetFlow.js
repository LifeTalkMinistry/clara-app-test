...
export default function useClaraBuyCheckBudgetFlow({ assistantContext = {} } = {}) {
      ...
      setState((current) => {
        ...
        try {
          if (current.step === "item") {
            return {
              ...
              messages: [
                ...current.messages,
                userMessage,
                createMessage("clara", `How much does ${answer} cost? Type the amount only. Example: ₱3,500`),
              ],
            };
          }
          ...
          if (current.step === "price") {
            ...
            const assessment = analyzeBuyCheckBudgetCoverage(current.item, price, assistantContext);
            ...
            if (coverage) {
              ...
              next.messages = [
                ...current.messages,
                userMessage,
                createMessage("clara", priceStepMessage(assessment)),
              ];
              return next;
            }
  ...
  const confirm = useCallback(async () => {
    ...
    const checking = createMessage(
      "clara",
      "Got it. I’m checking your wallet, every active budget, goals, emergency fund, schedule, Me profile, and memory now.",
    );
    ...
    setState({
      ...
      step: "diagnosis",
      busy: true,
      messages: [...snapshot.messages, createMessage("user", "Continue"), checking],
    });
      ...
      setState((current) => {
        ...
        return {
          ...
          messages: current.messages.map((entry) =>
            entry.id === checking.id ? { ...entry, text: "Your Buy Check report is ready." } : entry,
          ),
        };
      });
    } catch (error) {
  ...
  const checkAnother = useCallback(() => {
    setState(createInitialState(`buy-check-${Date.now()}-${Math.random().toString(36).slice(2)}`));
  }, []);
