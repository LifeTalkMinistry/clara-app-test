...
const CLUE_CARD_STYLES = [
  {
    card: 'border-cyan-100/24 bg-[radial-gradient(circle_at_88%_0%,rgba(34,211,238,0.24),transparent_38%),linear-gradient(135deg,rgba(8,145,178,0.28),rgba(15,23,42,0.60)_58%,rgba(30,41,59,0.42))]',
    orb: 'bg-cyan-200/[0.13]',
    iconBox: 'border-cyan-100/18 bg-cyan-100/[0.08]',
    label: 'text-cyan-50/95',
  },
  {
    card: 'border-violet-100/24 bg-[radial-gradient(circle_at_88%_0%,rgba(168,85,247,0.26),transparent_38%),linear-gradient(135deg,rgba(59,130,246,0.20),rgba(30,41,59,0.56)_48%,rgba(46,16,101,0.52))]',
    orb: 'bg-violet-200/[0.13]',
    iconBox: 'border-violet-100/18 bg-violet-100/[0.08]',
    label: 'text-violet-50/95',
  },
  {
    card: 'border-emerald-100/22 bg-[radial-gradient(circle_at_88%_0%,rgba(45,212,191,0.24),transparent_38%),linear-gradient(135deg,rgba(20,184,166,0.22),rgba(15,23,42,0.58)_56%,rgba(6,78,59,0.36))]',
    orb: 'bg-emerald-200/[0.12]',
    iconBox: 'border-emerald-100/18 bg-emerald-100/[0.08]',
    label: 'text-emerald-50/95',
  },
  {
    card: 'border-fuchsia-100/22 bg-[radial-gradient(circle_at_88%_0%,rgba(217,70,239,0.24),transparent_38%),linear-gradient(135deg,rgba(99,102,241,0.22),rgba(15,23,42,0.58)_55%,rgba(74,20,140,0.42))]',
    orb: 'bg-fuchsia-200/[0.12]',
    iconBox: 'border-fuchsia-100/18 bg-fuchsia-100/[0.08]',
            ...
            {word.split('').map((letter, letterIndex) => {
              ...
              return (
                <button
                  ...
                  aria-label={
                    selectedTile
                      ? `Remove letter ${shownLetter}`
                      : `Empty answer slot ${slotIndex + 1}`
                  }
      ...
      <main className='relative mx-auto flex h-[100dvh] max-h-[100dvh] w-full max-w-xl flex-col overflow-hidden px-[clamp(0.7rem,3.7vw,1rem)] pb-[max(0.55rem,env(safe-area-inset-bottom))] pt-[max(0.55rem,env(safe-area-inset-top))]'>
        <header className='flex shrink-0 items-center justify-between gap-2'>
          <button
            ...
            className='inline-flex h-[clamp(2.35rem,6.2dvh,2.75rem)] w-[clamp(2.35rem,6.2dvh,2.75rem)] shrink-0 items-center justify-center rounded-2xl border border-cyan-100/22 bg-white/[0.10] text-cyan-50 shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_10px_24px_rgba(0,0,0,0.22)] backdrop-blur-xl transition hover:bg-white/[0.14] active:scale-[0.98]'
          >
            <ArrowLeft className='h-5 w-5' />
          </button>

          <div className='min-w-0 flex-1 text-center'>
            <p className='text-[clamp(7px,1.2dvh,9px)] font-black uppercase tracking-[0.24em] text-cyan-100/70'>
              Money Game Mode
            </p>
            <h1 className='mt-0.5 truncate bg-gradient-to-r from-white via-cyan-100 to-violet-100 bg-clip-text text-[clamp(17px,2.6dvh,20px)] font-black tracking-[-0.03em] text-transparent'>
              {activeStageName}
            </h1>
          </div>
          ...
          {activePictureClues.map((clue, index) => {
            ...
            return (
              <div
                ...
                className={`relative flex min-h-0 flex-col items-center justify-center overflow-hidden rounded-[clamp(20px,4dvh,26px)] border p-[clamp(0.55rem,1.65dvh,1rem)] text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.13),0_18px_36px_rgba(0,0,0,0.18)] backdrop-blur-2xl ${clueStyle.card}`}
              >
        ...
        <section className='mt-[clamp(0.45rem,1.3dvh,0.85rem)] shrink-0 overflow-hidden rounded-[clamp(20px,4dvh,28px)] border border-cyan-100/22 bg-[radial-gradient(circle_at_12%_0%,rgba(34,211,238,0.16),transparent_36%),radial-gradient(circle_at_92%_10%,rgba(168,85,247,0.18),transparent_40%),linear-gradient(135deg,rgba(15,23,42,0.72),rgba(22,12,61,0.74))] p-[clamp(0.65rem,1.75dvh,1rem)] shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_18px_42px_rgba(0,0,0,0.26)] backdrop-blur-2xl'>
          <p
            ...
            className='text-center text-[clamp(7px,1.1dvh,9px)] font-black uppercase tracking-[0.22em] text-cyan-100/68'
          >
            Hidden money word
          </p>
          <AnswerSlots
            ...
            onRemoveLetter={removeSelectedLetter}
            isSolved={isSolved}
          />
          ...
          <form onSubmit={handleSubmit} className='mt-[clamp(0.5rem,1.25dvh,0.78rem)]'>
              ...
              {letterBank.map((tile) => {
                ...
                return (
                  <button
                    ...
                    className={`flex h-[clamp(1.75rem,4.7dvh,2.25rem)] w-[clamp(1.75rem,8.5vw,2.35rem)] items-center justify-center rounded-[11px] border text-[clamp(12px,1.95dvh,15px)] font-black shadow-[inset_0_1px_0_rgba(255,255,255,0.55),0_8px_18px_rgba(0,0,0,0.22)] transition active:scale-[0.96] ${
                      isUsed || isSolved
                        ? 'border-white/8 bg-white/[0.045] text-white/24 opacity-45'
                        : 'border-cyan-100/45 bg-[linear-gradient(135deg,rgba(255,255,255,0.96),rgba(224,242,254,0.94),rgba(237,233,254,0.94))] text-slate-900 hover:border-cyan-100 hover:bg-white'
                    }`}
                  >
                    {tile.letter}
                  </button>
            ...
            {showHint ? (
              <div className='mt-[clamp(0.45rem,1.1dvh,0.75rem)] rounded-[18px] border border-cyan-100/22 bg-cyan-100/[0.10] px-3 py-[clamp(0.5rem,1.2dvh,0.75rem)] text-[clamp(10.5px,1.55dvh,12px)] leading-snug text-cyan-50/82 shadow-[inset_0_1px_0_rgba(255,255,255,0.10)] backdrop-blur-xl'>
