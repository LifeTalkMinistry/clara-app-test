...
function ActivityList({ activity }) {
  ...
  return (
    <div className="space-y-2">
      {latest.map((item) => {
        const type = String(item?.type || "").toLowerCase();
        const isDecrease = type.includes("use") || type.includes("withdraw") || type.includes("expense") || type.includes("correction");
...
export default function EmergencyFundCard({ survivalExpense = 0, onSurvivalSaved, expanded = false, onToggleDetails }) {
  ...
  return (
    <>
      ...
      <EmergencyUseModal open={!isEmergencyFundUnconfigured && showUseModal} onClose={closeUseModal} amount={useAmount} setAmount={(value) => { setUseAmount(value); setUseError(""); }} reason={useReason} setReason={(value) => { setUseReason(value); setUseError(""); }} error={useError} saving={saving} onSave={emergencyActionType === "correction" ? applyEmergencyCorrection : useEmergencyMoney} currentReserve={savedAmount} actionType={emergencyActionType} setActionType={handleEmergencyActionTypeChange} orphanAllocation={orphanAllocation} onReverseOrphanAllocation={reverseOrphanAllocation} />
