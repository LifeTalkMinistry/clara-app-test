...
import EmergencyFundCardStorageWalletMoveConfirm from "@/components/fresh/main-dashboard/carousel/EmergencyFundCardStorageWalletMoveConfirm";
...
const broadcastFinanceRefresh = () => {
  ...
  REFRESH_EVENTS.forEach((eventName) => {
    window.dispatchEvent(new CustomEvent(eventName, { detail: { reason: "emergency-fund-added" } }));
  });
};
...
export default function EmergencyFundCardStorageWalletSynced(props) {
  ...
  useEffect(() => {
    if (!emergencyAllocationExpenses.length || !allocationActivities.length) return undefined;
    ...
    const syncProtectedMovement = async () => {
      for (const expense of emergencyAllocationExpenses) {
        ...
        if (!processingKey || !expenseId || processingRef.current.has(processingKey)) continue;
        ...
        const toWalletId =
          firstText(activity, ["storageWalletId", "storage_wallet_id", "reserveWalletId", "reserve_wallet_id", "linkedWalletId", "linked_wallet_id"]) ||
  ...
  return <EmergencyFundCardStorageWalletMoveConfirm {...props} />;
}
