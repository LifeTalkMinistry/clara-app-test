...
import { AlertTriangle, Check, Edit2, MinusCircle, Plus, RotateCcw, Shield, X } from "lucide-react";
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="theme-modal-card relative z-10 w-full max-w-md overflow-hidden rounded-3xl border border-white/[0.07] bg-[#061224]/95 shadow-2xl backdrop-blur-2xl">
        <div className="flex items-center justify-between border-b border-white/[0.06] p-4">
          <div>
            <p className="text-base font-semibold text-white">Add Emergency Fund</p>
            <p className="mt-0.5 text-xs text-white/45">Use any wallet as the funding source.</p>
          </div>
        ...
        <div className="space-y-4 p-4">
          ...
          <div>
            <label className="mb-2 block text-[11px] font-semibold uppercase tracking-[0.16em] text-white/45">Amount</label>
            <input type="number" min="0" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0" className="w-full rounded-2xl border border-white/[0.07] bg-black/[0.18] px-4 py-3 text-sm font-semibold text-white outline-none transition placeholder:text-white/30 focus:border-emerald-300/24" />
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[111] flex items-center justify-center p-4">
      ...
      <div className="theme-modal-card relative z-10 w-full max-w-md overflow-hidden rounded-3xl border border-white/[0.07] bg-[#061224]/95 shadow-2xl backdrop-blur-2xl">
        <div className="flex items-center justify-between border-b border-white/[0.06] p-4">
          <div>
            <p className="text-base font-semibold text-white">Use Emergency Fund</p>
            <p className="mt-0.5 text-xs text-white/45">Log protected money used for a real emergency.</p>
          </div>
        ...
        <div className="space-y-4 p-4">
          ...
          {error ? <div className="rounded-2xl border border-rose-300/16 bg-rose-400/[0.075] px-4 py-3 text-xs font-semibold text-rose-200">{error}</div> : null}
          <button type="button" onClick={onSave} disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-amber-300/22 bg-amber-400/[0.10] px-4 py-3 text-sm font-black text-amber-100 transition hover:bg-amber-400/[0.15] disabled:opacity-60">
            <MinusCircle className="h-4 w-4" />
            {saving ? "Logging..." : "Use Fund"}
          </button>
        </div>
      </div>
    </div>
              ...
              <div className="min-h-0 flex-1 overflow-hidden pt-1">
                <FinanceCardExpandedPanel className="h-full space-y-3 overflow-y-auto pr-1">
                  ...
                  <div className="rounded-2xl border border-white/[0.045] bg-black/[0.105] px-3.5 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.026)]">
                    <div className="mb-3 flex items-center justify-between gap-3"><span className="text-[10px] font-black uppercase tracking-[0.16em] text-white/34">Current setup</span><span className={`text-[11px] font-black ${status.text}`}>{safetyStage}</span></div>
                  ...
                  <div className="rounded-2xl border border-cyan-300/12 bg-cyan-400/[0.055] px-3.5 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.026)]">
                    ...
                    <select value={storageWalletId || ""} onChange={(event) => setStorageWallet(event.target.value)} disabled={saving || !safeWallets.length} className="w-full rounded-2xl border border-white/[0.07] bg-black/[0.18] px-3.5 py-3 text-xs font-black text-white outline-none transition focus:border-cyan-300/24 disabled:opacity-60">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1.5">
                    ...
                    <button type="button" onClick={() => setShowAddModal(true)} className="flex items-center justify-center gap-1.5 rounded-2xl border border-emerald-300/18 bg-emerald-400/[0.09] px-2 py-3.5 text-[12px] font-black text-emerald-200 shadow-[0_0_18px_rgba(52,211,153,0.08)] transition hover:bg-emerald-400/[0.13]"><Plus className="h-4 w-4" />Add</button>
