    ...
    return (
      <div className="space-y-4 pb-6">
        ...
        <section className={`${surfaceClass} p-5`}>
          ...
          <div className="mt-5 space-y-2.5">{["Financial records protected", "Each device starts with its own data", "No automatic device-to-device sync", "You choose when to transfer your data"].map((item) => <div key={item} className="flex items-center gap-2.5 text-sm font-semibold text-[#b8c9dc]"><Check className="h-4 w-4 shrink-0 text-[#58bfff]" /><span>{item}</span></div>)}</div>
          <p className="mt-4 text-xs leading-5 text-[#8298b2]">Your wallets, expenses, budgets, savings, transfers, transaction history, and AI context remain on this device unless you choose to back up or transfer them.</p>
