...
function getMoneyPersonalityResult(answers = {}) {
  const scores = Object.keys(PERSONALITY_RESULTS).reduce(
    (acc, key) => ({ ...acc, [key]: 0 }),
    {}
  );
    ...
    return (
      <div className="mt-5 rounded-[24px] border border-cyan-300/18 bg-cyan-300/[0.055] p-4">
        ...
        <p className="mt-3 text-xs leading-5 text-white/42">
          This is not a permanent label. CLARA can adjust as your behavior changes.
        </p>
        <button
          type="button"
          onClick={resetQuiz}
          className="mt-4 rounded-2xl border border-white/12 bg-white/[0.045] px-4 py-2 text-xs font-black text-white/62 transition active:scale-[0.98]"
        >
          Retake check
        </button>
      </div>
    ...
    <div className="mt-5 space-y-4">
      <div>
        <div className="flex items-center justify-between gap-3">
          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/60">
            Question {step + 1} of {MONEY_PERSONALITY_QUESTIONS.length}
          </p>
        ...
        <div className="mt-4 space-y-2.5">
          {question.options.map((option) => (
            <button
              ...
              className="w-full rounded-[18px] border border-white/10 bg-white/[0.035] px-3.5 py-3 text-left text-sm font-bold leading-5 text-white/72 transition hover:bg-white/[0.06] active:scale-[0.99]"
            >
              {option.text}
            </button>
          ))}
        ...
        <div className="rounded-[20px] border border-white/10 bg-white/[0.035] p-3">
          ...
          <div className="mt-3 flex gap-2">
            <button
              type="button"
              onClick={saveCustom}
              className="rounded-2xl border border-cyan-300/24 bg-cyan-300/[0.10] px-4 py-2 text-xs font-black text-cyan-50 transition active:scale-[0.98]"
            >
              Save custom
            </button>
            <button
              ...
              className="rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-2 text-xs font-black text-white/48 transition active:scale-[0.98]"
            >
              Cancel
            </button>
          </div>
...
function TextIdentityEditor({ field, profile, setProfile }) {
  ...
  const handleRefine = async () => {
    const roughText = String(value || "").trim();
    if (!roughText) {
    ...
    <div className="mt-5 rounded-[24px] border border-white/10 bg-white/[0.035] p-4">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        <button
          ...
          disabled={refining || !String(value || "").trim()}
          className={`inline-flex items-center gap-2 rounded-2xl border px-4 py-2 text-[11px] font-black uppercase tracking-[0.12em] transition active:scale-[0.98] ${
            refining || !String(value || "").trim()
              ? "cursor-not-allowed border-white/10 bg-white/[0.03] text-white/28"
              : "border-cyan-300/24 bg-cyan-300/[0.10] text-cyan-50 shadow-[0_0_20px_rgba(34,211,238,.08)] hover:bg-cyan-300/[0.14]"
          }`}
        >
        ...
        <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.14em] text-white/34">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h3 className="mt-3 text-2xl font-black leading-tight text-white">
              {field ? field.label : "What should CLARA know about your life?"}
            </h3>
            <p className="mt-2 text-sm leading-6 text-white/72">
              {field
                ? field.helper
                : "These details help CLARA guide your spending with your real life, values, and goals in mind."}
            </p>
        ...
        {field ? (
          ...
          <div className="mt-5 space-y-5">
            {Object.entries(sections).map(([sectionName, items]) => (
              <div key={sectionName}>
                <p className="mb-2.5 text-[10px] font-black uppercase tracking-[0.18em] text-white/34">
                  {sectionName}
                </p>
