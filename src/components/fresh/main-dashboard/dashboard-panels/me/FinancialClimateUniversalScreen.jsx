...
import { getLifeStageSignals } from "../../../../../life-stage-signals";
import {
...
function readStageProfile() {
  ...
  const saved = isLifeStageProfileConfigured(raw)
    ? readSelectedLifeStageProfile()
    : null;
  const stage = normalizeLifeStageKey(
    saved?.stage || DEFAULT_LIFE_STAGE_SELECTION
  );

  return {
    ...(saved || {}),
    stage,
    imageVariant: normalizeImageVariant(saved?.imageVariant || "default"),
  };
}
...
function readStageImages() {
  ...
  try {
    return JSON.parse(window.localStorage.getItem(STAGE_IMAGE_KEY) || "{}") || {};
  } catch {
    return {};
  }
}
...
function saveStageImages(images) {
  ...
  const next = images || {};
  try {
    window.localStorage.setItem(STAGE_IMAGE_KEY, JSON.stringify(next));
    notifyLifeStageUpdated({ kind: "images" });
  } catch {}
  return next;
}
...
function getQuestionKeys(stage, draft = {}) {
  ...
  return (
    questions.order || ["setup", "rhythm", "workload", "pressure", "coping", "goal"]
  ).filter(
...
function buildStageDraft(stageName, previous = {}) {
  ...
  const next = {
    stage,
    imageVariant: normalizeImageVariant(previous.imageVariant || "default"),
  };
  ...
  if (context) {
    return {
      ...
      summary:
        context.summary ||
        context.body ||
        getLifeStageStageContext(draft.stage),
    };
  }
...
function TrendSnapshotCard({ item, index, onClick }) {
  const trendPath = TREND_PATHS[item.trendType] || TREND_PATHS.wave;

  return (
    <button
      type="button"
      onClick={onClick}
      data-clara-trend-card="true"
      data-clara-trend-primary={index === 0 ? "true" : "false"}
      data-clara-trend-index={String(index + 1)}
      data-clara-snapshot-key={item.key || item.label}
      data-clara-snapshot-label={item.label}
      data-clara-snapshot-value={String(item.value)}
      data-clara-snapshot-status={item.status || "Active"}
      data-clara-snapshot-note={item.note || ""}
      data-clara-snapshot-insight={item.insight || ""}
      data-clara-snapshot-action={item.action || ""}
      data-clara-snapshot-trend={item.trendType || "wave"}
      className="h-full min-w-[116px] snap-start rounded-[16px] border border-white/[0.075] bg-[#071226]/66 p-3 text-left shadow-[0_0_22px_rgba(125,211,252,.10)] backdrop-blur-xl transition active:scale-[0.985]"
    >
      <p className="line-clamp-1 text-[9px] font-black tracking-tight text-white/56">
        {item.label}
      </p>
      ...
      <svg
        viewBox="0 0 92 34"
        className="mt-2 h-7 w-full overflow-visible"
        aria-hidden="true"
      >
        <path
          d={trendPath}
...
function SnapshotInsightRow({ label, children }) {
  return (
    <div className="rounded-[16px] border border-white/[0.075] bg-white/[0.035] px-3 py-2.5">
      <p className="text-[9px] font-black uppercase tracking-[0.14em] text-white/42">
        {label}
      </p>
      <p className="mt-1 text-[11px] font-semibold leading-5 text-white/72">
        {children}
      </p>
    </div>
  );
...
function DataDetailPanel({ trend, onClose }) {
  return (
    <div
      className="absolute inset-0 z-30 overflow-y-auto rounded-[28px] border border-white/[0.075] bg-[#050b1f]/94 p-4 shadow-[0_24px_80px_rgba(0,0,0,.42)] backdrop-blur-2xl [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      role="dialog"
      aria-modal="true"
      aria-label={`${trend.label} snapshot details`}
    >
      ...
      <div className="relative z-10 flex items-start justify-between gap-3">
        <div>
          <p className="text-[9px] font-black uppercase tracking-[0.18em] text-cyan-100/42">
            Data status
          </p>
      ...
      <div className="relative z-10 mt-5 rounded-[22px] border border-cyan-200/10 bg-cyan-300/8 p-4">
        <p className="text-[10px] font-black uppercase tracking-[0.16em] text-white/34">
          Behavioral distribution share
        </p>
      ...
      <div className="relative z-10 mt-3 space-y-2 rounded-[22px] border border-white/[0.065] bg-white/[0.03] p-3">
        <p className="px-1 text-[10px] font-black uppercase tracking-[0.16em] text-white/34">
          Snapshot basis
        </p>
        <SnapshotInsightRow label="Meaning">
          {trend.note || "This signal is part of the current life-stage distribution."}
        </SnapshotInsightRow>
        <SnapshotInsightRow label="Why it matters">
          {trend.insight || "This pattern helps CLARA understand the pressure behind the number."}
        </SnapshotInsightRow>
        <SnapshotInsightRow label="Next move">
          {trend.action || "Choose one small protection step for this signal."}
        </SnapshotInsightRow>
      </div>
    </div>
  );
...
function StageImagePanel({ stage, image, onApply, onClose }) {
  ...
  const handleUpload = (event) => {
    ...
    if (file.size > 3 * 1024 * 1024) {
      setError("Choose an image smaller than 3 MB so it can sync safely.");
      return;
    }
    ...
    reader.onerror = () => setError("CLARA could not read this image.");
  ...
  return (
    <div className="absolute inset-0 z-30 flex min-h-0 flex-col rounded-[28px] border border-white/[0.075] bg-[#050b1f]/92 p-4 shadow-[0_24px_80px_rgba(0,0,0,.42)] backdrop-blur-2xl">
      <div className="relative z-10 flex shrink-0 items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold leading-5 text-white/44">
            Use the default visual or upload your own image for this life stage.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative z-10 mt-4 min-h-0 flex-1 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {preview ? (
            ...
            <div className="grid h-full place-items-center text-center">
              <div>
                ...
                <p className="mt-2 text-xs font-black text-white/44">
                  Default stage visual
                </p>
              </div>
            </div>
          )}
        ...
        {error ? (
          <p className="mt-3 rounded-2xl border border-rose-300/15 bg-rose-400/10 px-3 py-2 text-[11px] font-bold leading-5 text-rose-100">
            {error}
          </p>
        ) : null}
      </div>
      ...
      <div className="relative z-10 mt-3 flex shrink-0 gap-2">
        <button
          type="button"
          onClick={onClose}
          className="flex-1 rounded-full border border-white/[0.075] bg-white/[0.035] px-4 py-3 text-xs font-black text-white/54 active:scale-95"
        >
          Cancel
        </button>
        <button
          ...
          className="flex flex-1 items-center justify-center gap-2 rounded-full bg-cyan-200 px-4 py-3 text-xs font-black text-slate-950 shadow-[0_10px_32px_rgba(125,211,252,.16)] active:scale-95"
        >
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="mx-auto inline-flex rounded-full border border-cyan-100/50 bg-cyan-50 px-3 py-1 text-[9px] font-black uppercase tracking-[0.18em] text-cyan-950 shadow-[0_0_20px_rgba(207,250,254,.22)]">
            Personalization needed
          </div>
          <div className="mx-auto mt-5 grid h-16 w-16 place-items-center rounded-[22px] bg-cyan-50 text-cyan-900 shadow-[0_16px_36px_rgba(34,211,238,.20)]">
            <Sparkles className="h-7 w-7" />
          </div>
          <h2 className="mt-5 text-[28px] font-black leading-[1.02] tracking-[-0.04em] text-white">
            Personalize your Money Profile
          </h2>
          <p className="mt-4 text-[13px] font-semibold leading-6 text-white/66">
            Set your life stage so CLARA can understand your real responsibilities,
            pressure points, and money behavior.
          </p>
          <button
            type="button"
            onClick={onSetup}
            className="mt-6 flex min-h-[56px] w-full items-center justify-center rounded-[22px] border border-cyan-100/10 bg-[linear-gradient(135deg,#164e63,#2563eb)] px-5 py-4 text-sm font-black uppercase tracking-[0.08em] text-white shadow-[0_18px_42px_rgba(37,99,235,.34),0_0_34px_rgba(14,116,144,.20)] active:scale-95"
          >
            SET LIFE STAGE NOW
          </button>
          <p className="mt-4 text-[11px] font-semibold leading-5 text-white/50">
            This helps CLARA personalize your reminders, spending signals, and
            financial safety insights.
          </p>
        </section>
      </div>
    </div>
...
function LifeStageSetupScreen({ profile, onClose, onSave }) {
  const [draft, setDraft] = useState(() =>
    buildStageDraft(profile.stage || DEFAULT_LIFE_STAGE_SELECTION, profile)
  );
  ...
  const progressPillIndex = Math.round(
    (stepIndex / Math.max(1, stepOrder.length - 1)) * 4
  );
  ...
  const insight = activeQuestionKey
    ...
    : null;
  const stageHero = getLifeStageHero(
    draft.stage,
    draft.imageVariant || "default"
  );
  ...
  const goNext = () => {
    ...
    const savedDraft = saveStageProfile({
      ...completedDraft,
      stage: normalizeLifeStageKey(draft.stage),
      imageVariant: normalizeImageVariant(
  ...
  return (
    <div className="fixed inset-0 left-1/2 z-[9999] flex h-[100dvh] w-full max-w-[430px] -translate-x-1/2 flex-col overflow-hidden bg-[#020817] px-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-[max(18px,env(safe-area-inset-top))] shadow-[0_24px_90px_rgba(0,0,0,.62),inset_0_0_0_1px_rgba(255,255,255,.04)]">
      ...
      <header className="relative z-10 shrink-0 overflow-hidden rounded-[32px] border border-cyan-200/18 bg-[#071226]/68 p-5 shadow-[0_22px_70px_rgba(0,0,0,.34),0_0_44px_rgba(34,211,238,.10),inset_0_1px_0_rgba(255,255,255,.08)] backdrop-blur-2xl">
        <div className="relative z-10 flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <h3 className="mt-5 max-w-[330px] text-[clamp(30px,8vw,40px)] font-black leading-[1.03] tracking-[-0.045em] text-white drop-shadow-[0_8px_24px_rgba(0,0,0,.35)]">
              {boardTitle}
            </h3>
            {boardSummary ? (
              <p className="mt-4 max-w-[350px] text-[13px] font-semibold leading-6 text-white/74">
                {boardSummary}
              </p>
            ) : null}
          </div>
      ...
      <main className="relative z-10 mt-5 min-h-0 flex-1 overflow-y-auto pr-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {activeQuestionKey ? (
          <div className="space-y-3.5 pb-4">
            <OptionGroup
              eyebrow={QUESTION_META[activeQuestionKey] || "Choose one"}
              value={draft[activeQuestionKey]}
              options={getLifeStageOptions(draft, activeQuestionKey) || []}
              displayValue={(option) =>
                getStageDisplayLabel(draft.stage, option)
              }
              onSelect={(value) =>
                setDraft((current) =>
      ...
      <footer className="relative z-10 mt-4 flex shrink-0 gap-4">
        <button
          type="button"
          onClick={goBack}
          className="flex min-h-[58px] flex-1 items-center justify-center gap-2 rounded-[22px] border border-cyan-200/20 bg-[#061327]/78 px-5 py-4 text-sm font-black text-white/86 shadow-[0_16px_38px_rgba(0,0,0,.28),inset_0_1px_0_rgba(255,255,255,.05)] backdrop-blur-xl active:scale-95"
        >
          {stepIndex <= 0 ? (
            "Cancel"
          ) : (
        ...
        <button
          type="button"
          onClick={goNext}
          className="flex min-h-[58px] flex-1 items-center justify-center gap-2 rounded-[22px] border border-white/20 bg-[linear-gradient(135deg,#67f8ff,#8bdcff_46%,#72a9ff)] px-5 py-4 text-sm font-black text-slate-950 shadow-[0_18px_42px_rgba(103,248,255,.24),0_0_34px_rgba(125,211,252,.22)] active:scale-95"
        >
...
function PressureSignalDock({ signals, selectedSignalId, onSelect }) {
  return (
    <div
...
export default function FinancialClimateUniversalScreen() {
  ...
  const [lifeStageConfigured, setLifeStageConfigured] = useState(() =>
    isLifeStageProfileConfigured()
  );
  const [selectedSignalId, setSelectedSignalId] = useState(null);
  const [supportMode, setSupportMode] = useState("awareness");

  const hero = useMemo(
    () =>
      getLifeStageHero(
        stageProfile.stage,
        stageProfile.imageVariant || "default"
      ),
    [stageProfile.stage, stageProfile.imageVariant]
  );
  const pressureSignals = useMemo(
    () => getLifeStageSignals(stageProfile.stage),
    [stageProfile.stage]
  );
  const supportCopy = useMemo(
    () =>
      getLifeStageGuidance(stageProfile.stage, {
        profile: stageProfile,
        signalId: selectedSignalId,
        mode: supportMode,
      }),
    [selectedSignalId, stageProfile, supportMode]
  );
  ...
  const hasExplicitGenderVariant = ["male", "female"].includes(
    normalizeImageVariant(stageProfile.imageVariant || "default")
  );
  const activeImage = hasExplicitGenderVariant
    ? hero.heroImage
    : customImage || hero.heroImage;
  const heroGlow =
    HERO_VISUALS[stageProfile.stage] || HERO_VISUALS["Young Professional"];
  const snapshotCards = snapshot.cards || [];

  useEffect(() => {
    const refreshFromStorage = () => {
      setStageProfile(readStageProfile());
      setStageImages(readStageImages());
      setLifeStageConfigured(isLifeStageProfileConfigured());
    };
  ...
  const persistProfilePatch = (patch) => {
    const next = saveStageProfile({ ...stageProfile, ...patch });
    setStageProfile(next);
    return next;
  };

  const handleGenderVariantChange = (variant) => {
    setShowHeroActions(false);
    persistProfilePatch({ imageVariant: normalizeImageVariant(variant) });
  };

  const handleImageApply = (image) => {
    const nextImages = { ...stageImages };
    if (image) nextImages[stageProfile.stage] = image;
    else delete nextImages[stageProfile.stage];

    saveStageImages(nextImages);
    setStageImages(nextImages);

    if (image) {
      persistProfilePatch({ imageVariant: "default" });
    }
  };
  ...
  if (showStageSetup) {
    return (
      <LifeStageSetupScreen
        profile={stageProfile}
        onClose={() => setShowStageSetup(false)}
        onSave={handleLifeStageSave}
      />
    );
  }

  if (!lifeStageConfigured) {
    return <LifeStageSetupGate onSetup={openLifeStageSetup} />;
  }

  return (
    <div
      data-clara-me-life-stage-root="true"
      className="relative flex h-full min-h-0 flex-col overflow-hidden rounded-[30px] bg-[#020817] px-3 pb-3 pt-1 shadow-[inset_0_0_0_1px_rgba(255,255,255,.035)]"
    >
      <section
        className={`relative min-h-0 flex-[1.25] overflow-hidden rounded-b-[30px] bg-gradient-to-br ${heroGlow} px-5 pb-5 pt-5 shadow-[0_22px_80px_rgba(0,0,0,.22)]`}
      >
        ...
        <div className="absolute bottom-0 right-0 h-full w-[56%] overflow-hidden">
          {activeImage ? (
            <img
              src={activeImage}
              alt={`${hero.title} stage background`}
              className="h-full w-full object-cover opacity-78 saturate-[.9]"
            />
          ) : (
        ...
        <div className="absolute left-4 top-4 z-20">
          ...
          {showHeroActions ? (
            <div
              data-clara-life-stage-actions-menu="true"
              className="absolute left-0 top-11 z-[80] flex w-[168px] flex-col overflow-hidden rounded-[18px] border border-white/[0.085] bg-[#071226]/90 p-1.5 shadow-[0_18px_54px_rgba(0,0,0,.38)] backdrop-blur-2xl"
            >
              <button
                type="button"
              ...
              <button
                type="button"
                onClick={() => handleGenderVariantChange("male")}
              ...
              <button
                type="button"
                onClick={() => handleGenderVariantChange("female")}
                className={`flex w-full items-center justify-start gap-2 rounded-[14px] px-3 py-2.5 text-left text-[11px] font-black uppercase tracking-[0.12em] transition hover:bg-white/[0.055] active:scale-[0.99] ${
                  normalizeImageVariant(
                    stageProfile.imageVariant || "default"
                  ) === "female"
        ...
        <div className="relative z-10 flex h-full max-w-[59%] flex-col justify-center pt-3">
          <p className="text-[9px] font-black uppercase tracking-[0.15em] text-white/52">
            {hero.label || "Your life stage"}
          </p>
          <h2 className="mt-2 text-[clamp(22px,7vw,31px)] font-black leading-[1.02] text-white drop-shadow-lg">
            {hero.title} <span className="text-[13px] text-amber-100/78">♛</span>
          </h2>
          <p className="mt-2 line-clamp-4 text-[12px] font-semibold leading-5 text-white/62">
            {hero.shortDescription || hero.contextText}
          </p>
        </div>
      </section>

      <section
        data-clara-support-card="true"
        data-clara-selected-signal={selectedSignalId || "default"}
        data-clara-signal-mode={supportMode}
        data-clara-signal-card-active={selectedSignalId ? "true" : "false"}
        className="relative min-h-0 overflow-hidden rounded-[24px] border border-white/[0.075] bg-[#071226]/56 p-3 backdrop-blur-xl"
      >
        <div className="flex h-full items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 line-clamp-3 text-[12px] font-semibold leading-5 text-white/56">
              {supportCopy.body}
            </p>
            {selectedSignalId ? (
              <span
                data-clara-solution-hint="true"
                className="mt-1.5 inline-flex rounded-full border border-cyan-100/12 bg-cyan-200/[0.06] px-2 py-1 text-[8px] font-black tracking-[0.04em] text-cyan-100/72"
              >
                {supportMode === "guidance"
                  ? "Guidance ready"
                  : "Reveal guidance"}
              </span>
            ) : null}
      ...
      <section
        data-clara-trend-snapshot="true"
        data-clara-snapshot-model={snapshot.model || "life-stage-engine"}
        className="flex min-h-0 flex-col overflow-hidden rounded-[24px] border border-white/[0.075] bg-[#071226]/50 p-3 backdrop-blur-xl"
      >
        <div
          data-clara-trend-header="true"
        ...
        >
          <div>
            ...
            <p className="mt-0.5 text-[10px] font-semibold text-white/36">
              {snapshot.subtitle || "Swipe the stage cards."}
            </p>
          </div>
          <Sparkles className="h-4 w-4 text-cyan-100/36" />
        </div>
        <div
          data-clara-trend-carousel="true"
          className="mt-3 flex min-h-0 flex-1 snap-x gap-2.5 overflow-x-auto overflow-y-hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
      ...
      {showImageSetup ? (
        <StageImagePanel
          ...
          onClose={() => setShowImageSetup(false)}
        />
      ) : null}
    </div>
  );
