      ...
      <div className="overflow-hidden rounded-[30px] border border-white/12 bg-slate-950/94 shadow-[0_-24px_80px_rgba(0,0,0,.45)] backdrop-blur-2xl">
        ...
        <div className="space-y-3 p-4">
          <div className="max-w-[88%] rounded-[22px] bg-white/[0.07] px-4 py-3 text-sm font-semibold leading-6 text-white/76">
            {current
              ? `I currently understand this as: “${current}.” Tell me the correct version if this changed.`
              : "I do not have this yet. Tell me what CLARA should remember."}
          </div>
          {savedText ? (
            <>
              ...
              <div className="max-w-[88%] rounded-[22px] bg-white/[0.07] px-4 py-3 text-sm font-semibold leading-6 text-white/76">
                Saved. I’ll use this when giving you money guidance.
              </div>
            </>
          ) : null}
        </div>
        ...
        <form onSubmit={submit} className="border-t border-white/10 p-3">
          <div className="flex items-center gap-2 rounded-[22px] border border-white/10 bg-white/[0.055] px-3 py-2">
            <input
              ...
              placeholder="Tell CLARA what to remember..."
            />
            <button
              type="submit"
              disabled={!draft.trim()}
          ...
          {!activeDrawer ? (
            <>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-100/42">Personal Cabinet</p>
                  <h2 className="mt-1.5 text-3xl font-black leading-none text-white">Me</h2>
                  <p className="mt-3 max-w-[24rem] text-sm font-semibold leading-6 text-white/52">
                    Private memory drawers for your financial behavior and life context.
                  </p>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    <span className="rounded-full bg-white/[0.05] px-3 py-1 text-[11px] font-black text-white/42">{saved}/{total} learned</span>
                    <span className="rounded-full bg-white/[0.04] px-3 py-1 text-[11px] font-bold text-white/34">{dateLabel(memory.updatedAt)}</span>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={refresh}
                  className="grid h-10 w-10 shrink-0 place-items-center rounded-[18px] border border-white/8 bg-white/[0.04] text-white/55 active:scale-95"
              ...
              <div className="mt-5 grid flex-1 grid-cols-1 gap-4">
                {drawers.map((drawer) => (
                  <button
                    ...
                    className="group rounded-[26px] border border-white/8 bg-white/[0.028] p-3.5 text-left transition active:scale-[0.985]"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="text-[22px] font-black leading-tight text-white">{drawer.title}</h3>
                        <p className="mt-2 text-xs font-semibold leading-5 text-white/36">{drawer.subtitle}</p>
                      </div>
                      <ChevronRight className="mt-2 h-4 w-4 shrink-0 text-white/28" />
                    </div>
                    <div className="mt-3 inline-flex rounded-full bg-white/[0.04] px-3 py-1 text-[11px] font-black text-white/36">{drawer.saved}/{drawer.total} saved</div>
                  </button>
                ))}
              </div>
            </>
            ...
            <>
              <div className="flex items-start gap-3">
                <button
                  ...
                  className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-white/8 bg-white/[0.045] text-white/62 active:scale-95"
                  aria-label="Back to drawers"
                >
                  <ArrowLeft className="h-4 w-4" />
                </button>
                <div className="min-w-0 flex-1">
                  <h2 className="text-[26px] font-black leading-tight text-white">{activeDrawer.title}</h2>
                  <p className="mt-2 text-sm font-semibold leading-6 text-white/40">{activeDrawer.subtitle}</p>
                </div>
                <span className="shrink-0 rounded-full bg-white/[0.05] px-3 py-1 text-[11px] font-black text-white/42">{activeDrawer.saved}/{activeDrawer.total}</span>
              </div>

              <div className="mt-6 rounded-[26px] border border-white/8 bg-white/[0.026] px-4">
