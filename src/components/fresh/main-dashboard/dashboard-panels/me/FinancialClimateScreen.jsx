...
import { clean } from "./meMemoryUtils";

function valueOf(memory, key) {
  return clean(memory?.items?.[key]?.value);
}

function readClimate(memory) {
  const income = valueOf(memory, "incomePattern");
  const living = valueOf(memory, "livingSituation");
  const work = valueOf(memory, "workType");
  const responsibilities = valueOf(memory, "responsibilities");
  const goal = valueOf(memory, "mainFinancialGoal");
  const emotional = valueOf(memory, "emotionalStateTrend");
  ...
  return { pressure, summary, similar, focus };
}

export default function FinancialClimateScreen({ memory, saved, total }) {
  const climate = readClimate(memory);
    ...
    <div className="relative overflow-hidden rounded-[30px] border border-cyan-200/14 bg-[linear-gradient(145deg,rgba(255,255,255,.075),rgba(255,255,255,.025)_48%,rgba(16,185,129,.055))] p-5 shadow-[0_18px_60px_rgba(0,0,0,.25)] backdrop-blur-2xl">
      ...
      <div className="relative">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200/14 bg-emerald-300/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-emerald-100/78">
              <Sparkles className="h-3.5 w-3.5" /> Adaptive climate
            </div>
          ...
          <div className="rounded-[18px] border border-white/10 bg-white/[0.05] px-3 py-2 text-right">
            <p className="text-[10px] font-black uppercase tracking-[0.14em] text-white/34">Signals</p>
            <p className="mt-1 text-sm font-black text-white/76">{saved}/{total}</p>
          </div>
        </div>
        ...
        <div className="mt-5 rounded-[22px] border border-white/8 bg-white/[0.035] p-4">
          ...
          <div className="mt-3 space-y-2">
            {climate.similar.map((item) => (
              <div key={item} className="flex gap-2 text-xs font-semibold leading-5 text-white/58">
                <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-200/72" />
                <span>{item}</span>
              </div>
            ))}
          </div>
        ...
        <div className="mt-3 rounded-[22px] border border-white/8 bg-white/[0.026] p-4">
          ...
          <div className="mt-3 space-y-2">
            {climate.focus.map((item) => (
              <p key={item} className="text-xs font-semibold leading-5 text-white/50">• {item}</p>
            ))}
          </div>
        </div>
      </div>
