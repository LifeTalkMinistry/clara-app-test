...
export default function GuidedWalletCreationModal({
  ...
  const effectiveIncomeSourceId = String(financeForm.incomeSourceId || incomeSources[0]?.id || "");
  ...
  const selectedIncomeSource = useMemo(
    () => incomeSources.find((source) => String(source.id) === effectiveIncomeSourceId) || null,
    [effectiveIncomeSourceId, incomeSources]
  );
  ...
  const setMode = (mode) => {
    setMoneyMode(mode);
    const currentAmount = String(financeForm.amount ?? financeForm.startingBalance ?? "");
    const editableAmount = toNumber(currentAmount) === 0 ? "" : currentAmount;

    updateForm({
      startingBalanceMode: mode,
      incomeSourceId: mode === "income_hub" ? effectiveIncomeSourceId : financeForm.incomeSourceId,
      amount: mode === "skip" ? "0" : editableAmount,
      startingBalance: mode === "skip" ? "0" : editableAmount,
    });
  };
  ...
  const canMoneyContinue =
    ...
    (moneyMode === "income_hub" && incomeSources.length > 0 && Boolean(effectiveIncomeSourceId) && amountNumber > 0);
  const incomeHubBusy = moneyMode === "income_hub" && incomeSourcesLoading && incomeSources.length === 0;
  const disabled =
    loading ||
    incomeHubBusy ||
  ...
  const primary = () => {
    ...
    if (step === "money_setup") {
      if (!canMoneyContinue) return null;
      if (moneyMode === "income_hub" && !financeForm.incomeSourceId && effectiveIncomeSourceId) {
        updateForm({ incomeSourceId: effectiveIncomeSourceId });
      }
              ...
              <div className="flex h-full min-h-0 flex-col space-y-3">
                ...
                {search.trim() ? (
                  ...
                  <div className="flex min-h-0 flex-1 flex-col space-y-3">
                    ...
                    <div className="min-h-0 flex-1 rounded-[24px] border border-white/[0.09] bg-white/[0.035] p-3">
