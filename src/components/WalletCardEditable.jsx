...
const getWalletBalanceValue = (wallet) =>
  toNumber(
...
function getWalletMessage(topWallet, walletCount) {
  ...
  if (topWallet) {
    return `${topWallet.name || "Top wallet"} currently holds ${fmt(
      getWalletBalanceValue(topWallet)
    )}.`;
  }
  return "Your wallets are ready for tracking and movement.";
}

export default function WalletCardEditable({
  wallets = [],
  walletMoney = 0,
  ...
  const [editForm, setEditForm] = useState({ name: "", type: "cash", balance: "" });
  ...
  const openEditWallet = (wallet) => {
    ...
    setEditForm({
      ...
      balance: String(getWalletBalanceValue(wallet)),
    });
  };
  ...
  const closeEditWallet = () => {
    ...
    setEditForm({ name: "", type: "cash", balance: "" });
  };
  ...
  const handleSaveWalletEdit = async () => {
    ...
    try {
      ...
      const nextBalance = toNumber(editForm.balance);
  ...
  return (
    <div className={`relative flex h-full min-h-[inherit] flex-col overflow-hidden rounded-3xl border shadow-2xl transition-all duration-200 ${themeClasses.border} ${status.ring}`}>
        ...
        <div className="flex min-h-0 flex-1 flex-col justify-between">
          <div>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className={`text-base font-semibold tracking-tight ${themeClasses.title}`}>Wallets</p>
                    <p className={`mt-0.5 text-[11px] font-medium ${themeClasses.body}`}>Track your available money across accounts</p>
                  </div>
            ...
            <div className="mb-3">
              ...
              <p className={`mt-2 line-clamp-1 min-h-[20px] max-w-[28rem] text-xs font-medium leading-relaxed ${themeClasses.body}`}>{message}</p>
              <p className={`mt-1 text-[11px] ${themeClasses.muted}`}>Total money spread across your wallet system.</p>
            </div>

            <div className="mb-3 grid grid-cols-3 gap-2">
              <div className={`rounded-2xl border px-2.5 py-2 text-center backdrop-blur-[2px] ${themeClasses.glass}`}>
                <p className={`mb-1 text-[10px] font-semibold uppercase tracking-[0.16em] ${themeClasses.muted}`}>Wallets</p>
                <p className={`text-xs font-bold ${themeClasses.title}`}>{wallets.length}</p>
              </div>
              <div className={`rounded-2xl border px-2.5 py-2 text-center backdrop-blur-[2px] ${themeClasses.glass}`}>
                <p className={`mb-1 text-[10px] font-semibold uppercase tracking-[0.16em] ${themeClasses.muted}`}>Top Wallet</p>
                <p className={`truncate text-xs font-bold ${themeClasses.title}`}>{topWallet?.name || "None"}</p>
              </div>
              <div className={`rounded-2xl border px-2.5 py-2 text-center backdrop-blur-[2px] ${themeClasses.glass}`}>
                <p className={`mb-1 text-[10px] font-semibold uppercase tracking-[0.16em] ${themeClasses.muted}`}>Activity</p>
                <p className={`text-xs font-bold ${themeClasses.title}`}>{walletPreviewTransactions.length}</p>
              </div>
            </div>
          </div>

          <button type="button" onClick={onToggleDetails} className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-black/15 px-3 py-2.5 text-sm text-white/85 backdrop-blur-sm transition hover:bg-white/10">
            ...
            {wallets.length ? (
              <div className="space-y-2">
                {visibleWallets.map((wallet, index) => (
                  <div key={wallet.id || `${wallet.name}-${index}`} className="rounded-2xl border border-white/10 bg-black/20 p-3 backdrop-blur-[2px] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        <p className="text-sm font-bold text-white">{fmt(getWalletBalanceValue(wallet))}</p>
                        <button
                          type="button"
                          onClick={(event) => {
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button type="button" onClick={() => onAddMoney?.(wallet)} disabled={financeActionLoading} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-white/85 transition hover:bg-white/10 hover:text-white disabled:opacity-50">Add</button>
                      <button type="button" onClick={() => onTransferMoney?.(wallet)} disabled={financeActionLoading} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-white/85 transition hover:bg-white/10 hover:text-white disabled:opacity-50">Transfer</button>
                      <button type="button" onClick={() => onMoveWallet?.(wallet.id, -1)} disabled={financeActionLoading} className="rounded-xl border border-white/10 bg-white/5 p-2 text-white/80 transition hover:bg-white/10 disabled:opacity-50" aria-label="Move wallet up"><ArrowUp className="h-4 w-4" /></button>
                      <button type="button" onClick={() => onMoveWallet?.(wallet.id, 1)} disabled={financeActionLoading} className="rounded-xl border border-white/10 bg-white/5 p-2 text-white/80 transition hover:bg-white/10 disabled:opacity-50" aria-label="Move wallet down"><ArrowDown className="h-4 w-4" /></button>
                      <button type="button" onClick={() => onDeleteWallet?.(wallet.id)} disabled={financeActionLoading} className="rounded-xl border border-rose-400/20 bg-rose-500/10 p-2 text-rose-200 transition hover:bg-rose-500/20 disabled:opacity-50" aria-label="Delete wallet"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))}
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/20 p-3 backdrop-blur-[2px] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                    ...
                    <div className="mt-3 space-y-2">
                      {visibleTransactions.map((item, index) => (
                        <div key={item.id || `${item.type}-${index}`} className="flex items-center justify-between gap-3 rounded-xl border border-white/6 bg-white/[0.03] px-3 py-2">
                          <div className="min-w-0 flex-1">
              ...
              <div className={`rounded-2xl border border-dashed p-4 text-center ${themeClasses.softGlass}`}>
                ...
                <p className={`mt-2 text-sm leading-6 ${themeClasses.muted}`}>Create your first wallet so your money is organized and easier to track.</p>
              </div>
            )}

            <div className="grid grid-cols-1 gap-2">
              <button type="button" onClick={onCreateWallet} className={`flex items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-sm font-semibold transition ${themeClasses.action}`}>
      ...
      {editingWallet && (
        <div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/70 px-4 py-5 backdrop-blur-sm sm:items-center">
          <div className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-[30px] border border-white/10 bg-[#030914] text-white shadow-[0_30px_90px_rgba(0,0,0,0.72),0_0_44px_rgba(16,185,129,0.16)] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <div className="relative border-b border-white/10 px-5 py-5">
              ...
              <div className="pr-12">
                <div className="mb-3 inline-flex items-center rounded-full border border-emerald-300/15 bg-emerald-400/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-100/85">Wallet setup</div>
                <h3 className="text-xl font-bold tracking-tight text-white">Edit wallet</h3>
                <p className="mt-2 text-sm leading-6 text-white/65">Update the name, type, and balance of this money container.</p>
              </div>
            </div>

            <div className="space-y-5 px-5 py-5">
              <div className="rounded-[26px] border border-emerald-300/15 bg-gradient-to-br from-emerald-400/[0.10] via-white/[0.045] to-cyan-400/[0.08] p-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="mt-2 text-sm font-bold text-emerald-100">{fmt(toNumber(editForm.balance))}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <p className="text-sm font-semibold text-white/86">Wallet balance</p>
                <div className="relative">
                  ...
                  <input type="number" min="0" inputMode="decimal" value={editForm.balance} onChange={(event) => setEditForm((prev) => ({ ...prev, balance: event.target.value }))} placeholder="0.00" className="h-12 w-full rounded-2xl border border-white/15 bg-white/[0.07] pl-9 pr-4 text-white outline-none placeholder:text-white/35 focus:border-emerald-300/45 focus:ring-2 focus:ring-emerald-400/20" />
                </div>
                <p className="text-xs leading-5 text-white/45">This updates the wallet amount shown on the dashboard.</p>
              </div>
