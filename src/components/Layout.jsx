...
export default function Layout({ children }) {
  ...
  const hintLabel = isAnalyticsPath(location.pathname) ? "Swipe down to return from analytics" : "Swipe down to go back";
  ...
  useEffect(() => {
    ...
    const startLongPress = (event) => {
      ...
      longPressStartRef.current = { x: event.clientX, y: event.clientY, target };
    ...
    const endLongPress = (event) => {
      ...
      if (longPressTriggeredRef.current) {
        ...
        window.setTimeout(() => { longPressTriggeredRef.current = false; }, 80);
      }
    };
  ...
  useEffect(() => {
    const handleAssistantOpen = (event) => openAssistant(event?.detail?.mode || "voice");
    const handleManualExpenseOpen = () => handleOpenQuickAdd();
    const handleBuyCheckExpenseOpen = (event) => {
      setQuickAddPrefill(event?.detail || null);
    ...
    window.addEventListener("clara:open-buy-check-expense", handleBuyCheckExpenseOpen);
    return () => {
      ...
      window.removeEventListener("clara:open-buy-check-expense", handleBuyCheckExpenseOpen);
    };
  ...
  const handleMotionTouchEnd = useCallback((event) => {
    ...
    const shouldClose = startedAtTop && deltaY > 0 && deltaY > deltaX * 1.15 && (dragY > 128 || deltaY > 172 || (velocity > 0.72 && deltaY > 54));
    ...
    if (isDraggingMotion || dragY > 0) {
      ...
      setDragY(0);
      window.setTimeout(() => setIsReboundingMotion(false), 260);
    }
  ...
  return (
    <div className="theme-page-shell flex h-screen overflow-hidden text-white">
      <style>{`
        ...
        @keyframes claraOverlayOut { 0% { opacity: 1; backdrop-filter: blur(10px); } 100% { opacity: 0; backdrop-filter: blur(0px); } }
        @keyframes claraHintInOut { 0% { opacity: 0; transform: translate(-50%, -8px); } 18% { opacity: .5; transform: translate(-50%, 0); } 76% { opacity: .5; transform: translate(-50%, 0); } 100% { opacity: 0; transform: translate(-50%, -8px); } }
        @keyframes claraContentSettle { 0%, 42% { opacity: .72; transform: translateY(10px); } 100% { opacity: 1; transform: translateY(0); } }
        .clara-motion-stage { transform-origin: center center; backface-visibility: hidden; perspective: 1600px; will-change: transform, opacity, filter, border-radius, box-shadow; overflow: hidden; position: relative; z-index: 30; touch-action: pan-y; }
        .clara-motion-stage-in { animation: claraMotionExpandIn 680ms cubic-bezier(.16,.92,.22,1) both; }
        .clara-motion-stage-in > * { animation: claraContentSettle 780ms cubic-bezier(.16,.92,.22,1) both; }
        .clara-motion-stage-dragging { animation: none !important; transform: translate3d(0, var(--clara-drag-y), 0) scale(var(--clara-drag-scale)) rotateX(calc(var(--clara-drag-y) * .025deg)); border-radius: var(--clara-drag-radius); box-shadow: 0 26px 90px rgba(0,0,0,0.28); transition: none; }
        .clara-motion-stage-rebound { animation: none !important; transform: translate3d(0, 0, 0) scale(1) rotateX(0deg); border-radius: 0px; transition: transform 260ms cubic-bezier(.18,.9,.24,1), border-radius 260ms cubic-bezier(.18,.9,.24,1), box-shadow 260ms ease; }
        .clara-motion-stage-out { animation: claraMotionCollapseOut 380ms cubic-bezier(.4,0,.2,1) both; pointer-events: none; }
        .clara-motion-overlay { animation: claraOverlayIn 360ms ease-out both; background: radial-gradient(circle at 50% 20%, rgba(255,255,255,0.08), transparent 30%), rgba(0,0,0,0.22); transition: opacity 120ms linear, backdrop-filter 120ms linear; }
        .clara-motion-overlay-out { animation: claraOverlayOut 380ms ease-in both; }
        .clara-motion-hint { animation: claraHintInOut 2600ms ease both; }
      `}</style>

      {loading && <div className="fixed inset-0 z-[999] flex items-center justify-center bg-[color:var(--theme-background)]"><div className="h-8 w-8 animate-spin rounded-full border-4 border-green-500/20 border-t-green-500" /></div>}
      {activeMotionPage && <div style={!motionClosing ? { opacity: overlayOpacity } : undefined} className={`pointer-events-none fixed inset-0 z-20 clara-motion-overlay ${motionClosing ? "clara-motion-overlay-out" : ""}`} />}
      {activeMotionPage && <div className="pointer-events-none fixed left-1/2 top-[calc(env(safe-area-inset-top)+0.75rem)] z-40 h-1.5 w-12 -translate-x-1/2 rounded-full bg-[color:var(--theme-text)]/35 transition-opacity duration-150" style={{ opacity: Math.max(0.18, 1 - dragProgress * 1.4) }} />}
      {activeMotionPage && showSwipeHint && !motionClosing && !isDraggingMotion && <div className="clara-motion-hint pointer-events-none fixed left-1/2 top-[calc(env(safe-area-inset-top)+2.05rem)] z-40 -translate-x-1/2 rounded-full border border-[color:var(--theme-border)] bg-[color:var(--theme-card)]/45 px-3.5 py-1.5 text-[11px] font-medium tracking-wide text-[color:var(--theme-text)]/50 shadow-[0_12px_32px_rgba(0,0,0,0.18)] backdrop-blur-xl">{hintLabel}</div>}
      ...
      <div className="relative flex min-w-0 flex-1 flex-col">
        <main
          ...
          className={`flex-1 overflow-y-auto pb-24 pt-3 ${activeMotionPage ? `clara-motion-stage ${motionClosing ? "clara-motion-stage-out" : isDraggingMotion ? "clara-motion-stage-dragging" : isReboundingMotion ? "clara-motion-stage-rebound" : "clara-motion-stage-in"}` : ""}`}
        >
          {children}
        </main>
      </div>

      <QuickAddModal open={quickAddOpen} onClose={() => setQuickAddOpen(false)} userEmail={user?.email} initialExpenseData={quickAddPrefill} />
      <AdsModal open={adsModalOpen} onClose={() => setAdsModalOpen(false)} userEmail={user?.email} />
      {assistantOpen && <Suspense fallback={null}><ClaraAssistantPanel open={assistantOpen} mode={assistantMode} onClose={() => setAssistantOpen(false)} /></Suspense>}
    </div>
  );
}
