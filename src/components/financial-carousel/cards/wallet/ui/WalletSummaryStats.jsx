...
export default function WalletSummaryStats({
  ...
  return (
    <>
      <div className='mb-3'>
        <p className={`text-[32px] font-black leading-none tracking-[-0.035em] ${status.text}`}>
          {fmt(walletMoney)}
        </p>
        <p className='mt-2 line-clamp-1 min-h-[18px] max-w-[28rem] text-xs font-semibold leading-relaxed text-white/82'>
          {message}
        </p>
      </div>
      ...
      <div className='mb-3 grid grid-cols-3 gap-2'>
        ...
        <div className={`rounded-2xl px-2.5 py-2.5 text-center ${glassPanel}`}>
          ...
          <p className='mt-1.5 text-[9px] font-bold uppercase tracking-[0.16em] text-white/45'>Top Wallet</p>
        </div>

        <div className={`rounded-2xl px-2.5 py-2.5 text-center ${glassPanel}`}>
          <p className='text-sm font-black leading-none text-white'>{walletPreviewTransactions.length}</p>
          <p className='mt-1.5 text-[9px] font-bold uppercase tracking-[0.16em] text-white/45'>Activity</p>
        </div>
      </div>
    </>
  );
