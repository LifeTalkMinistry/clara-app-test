...
function BudgetInsightCard({ driftState, outsidePlanSpent, onOpenDetails }) {
  return (
    <button
      ...
      className={`relative w-full cursor-pointer overflow-hidden rounded-[20px] border px-3.5 py-3.5 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.055)] backdrop-blur-sm transition hover:border-white/[0.16] hover:bg-white/[0.06] active:scale-[0.992] ${driftState.tone}`}
      ...
      <div className="relative flex h-[calc(100vh-245px)] w-full max-w-[352px] flex-col overflow-hidden rounded-[28px] border border-emerald-100/[0.12] bg-[linear-gradient(135deg,rgba(6,42,52,0.97),rgba(14,24,61,0.985)_48%,rgba(50,30,91,0.97))] shadow-[0_24px_80px_rgba(0,0,0,0.58),0_0_42px_rgba(45,212,191,0.09)]">
        ...
        <div className="relative flex items-start justify-between gap-3 border-b border-white/[0.055] px-4 pb-3.5 pt-4">
          <div className="min-w-0">
            <p className="text-[9px] font-black uppercase tracking-[0.22em] text-teal-100/55">
              Budget documentation
            </p>
            <h3 className="mt-1 text-[17px] font-black leading-tight text-white/92">
        ...
        <div className="relative min-h-0 flex-1 overflow-y-auto px-4 pb-4 pt-3.5">
          <div className="grid grid-cols-3 gap-2">
            {summaryMetrics.map(([label, value]) => (
              <div
                key={label}
                className="rounded-2xl border border-white/[0.065] bg-black/[0.12] px-2 py-2 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
              >
                <p className="text-[8px] font-black uppercase tracking-[0.13em] text-white/36">
                  {label}
                </p>
                <p className="mt-1 truncate text-[11px] font-black text-white/82">
                  {value}
                </p>
              </div>
            ))}
          ...
          {items.length ? (
            <div className="mt-3.5 space-y-2.5">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="rounded-[20px] border border-white/[0.07] bg-white/[0.045] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <span className="rounded-full border border-white/[0.08] bg-black/[0.16] px-2 py-0.5 text-[8px] font-black uppercase tracking-[0.15em] text-teal-100/64">
                        {item.label}
                      </span>
                      <p className="mt-2 truncate text-sm font-black text-white/88">
                        {item.title}
                      </p>
                    ...
                    <p className="shrink-0 text-sm font-black text-white/90">
                      {fmt(item.amount)}
                    </p>
                  </div>

                  {item.note ? (
                    <p className="mt-2 rounded-2xl border border-white/[0.055] bg-black/[0.10] px-3 py-2 text-[11px] font-semibold leading-5 text-white/50">
                      {item.note}
                    </p>
                  ) : null}
                </div>
            ...
            <div className="mt-3.5 rounded-[22px] border border-white/[0.065] bg-black/[0.12] p-4 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
              <p className="text-sm font-black text-white/84">No records to review.</p>
