...
function replaceOnce(source, search, replacement, label) {
  ...
  if (first < 0) {
    throw new Error(`[savings-managed-balance] Missing source contract: ${label}`);
  }

  if (first !== last) {
    throw new Error(`[savings-managed-balance] Ambiguous source contract: ${label}`);
  }
...
function transformGoalDetail(source) {
  ...
  if (start < 0 || end < 0) {
    throw new Error("[savings-managed-balance] GoalDetail boundaries were not found.");
  }
  ...
  const [correctionError, setCorrectionError] = useState("");`,
    "GoalDetail managed-balance state",
  );
  ...
  return <>`,
    "GoalDetail managed-balance handlers",
  );

  detail = replaceOnce(
    detail,
    '</div></div><InfoBlock title="Saved in">',
    `</div></div>{hasBalanceMismatch ? <div className="rounded-2xl border border-rose-300/25 bg-rose-400/[0.1] p-4"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-rose-200" /><div><p className="text-sm font-bold text-rose-50">Savings balance needs correction</p><p className="mt-1 text-xs leading-5 text-rose-100/70">This goal records {fmt(saved)}, but this wallet can safely support only {fmt(safeWalletCapacity)} after other protected money.</p></div></div><Button type="button" onClick={() => { setCorrectedAmount(String(Math.min(saved, safeWalletCapacity))); setCorrectionReason(""); setCorrectionError(""); setCorrectionOpen(true); }} className="mt-3 h-9 w-full rounded-xl border border-rose-200/20 bg-rose-400/15 text-xs font-bold text-rose-50 hover:bg-rose-400/20">Correct Balance</Button></div> : null}<InfoBlock title="Saved in">`,
    "GoalDetail mismatch warning",
  );

  detail = replaceOnce(
    detail,
    '<span className={entry.type === "use" ? "text-amber-200" : "text-green-200"}>{entry.type === "use" ? "-" : "+"}{fmt(entry.amount)}</span>',
  ...
  detail = replaceOnce(
    ...
    "GoalDetail managed-balance dialogs",
  );

  return source.slice(0, start) + detail + source.slice(end);
}

export function transformSavingsGoalsManagedBalance(input) {
  let source = input;
  ...
  source = replaceOnce(
    ...
    if (form.wallet_id === "__no_wallets__") return setFormError("Choose a valid wallet.");`,
    "existing saved amount is metadata-read-only",
  );
  ...
  const handleUseSavings = async (goal, amount, reason) => {`,
    "managed-balance parent actions",
  );
  ...
  source = replaceOnce(
    ...
    "GoalFormDialog managed-balance callback",
  );
  ...
  source = replaceOnce(
    ...
    "GoalDetail managed-balance props",
  );
  ...
  source = replaceOnce(
    ...
    "GoalFormDialog protected savings field",
  );
...
export function savingsGoalManagedBalancePlugin() {
  return {
    name: "clara-savings-goal-managed-balance",
    enforce: "pre",
    transform(code, id) {
      const normalizedId = String(id || "").replaceAll("\\", "/").split("?")[0];
      if (!normalizedId.endsWith(SAVINGS_GOALS_FILE_SUFFIX)) return null;
      return { code: transformSavingsGoalsManagedBalance(code), map: null };
    },
  };
}
