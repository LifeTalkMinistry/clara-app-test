...
function getSingleParentSupportCopy(profile = {}) {
  ...
  if (hasAny(text, ["child essentials", "school", "childcare", "child needs", "child stability", "future"])) {
    return { title: "Your priority is protection.", body: "Child needs can make every money decision feel important because food, school, health, and routine all connect to stability." };
  }
  if (hasAny(text, ["emergency", "health", "weak safety", "borrow", "debt"])) {
    return { title: "The safety gap is the pressure.", body: "When one surprise can disrupt the whole month, even small buffers can matter more than big plans." };
  }
  if (hasAny(text, ["guilt", "sacrifice", "own needs", "exhausted", "care and work", "busy"])) {
    return { title: "Care is costing energy too.", body: "Parenting pressure can make personal needs feel optional, but energy and stability are part of protecting the household." };
  }
...
function getFullTimeEarnerSupportCopy(profile = {}) {
  ...
  if (hasAny(text, ["payday", "cutoff", "salary", "bills arrive", "strong then fades"])) {
    return { title: "Salary needs a rhythm.", body: "Stable income can still feel tight when payday, bills, food, subscriptions, and daily spending do not move in the same direction." };
  }
  if (hasAny(text, ["reward", "convenience", "stress spending", "tired", "burnout", "shift", "commute"])) {
    return { title: "Routine fatigue is showing up.", body: "Work can make spending feel like recovery, especially when the day leaves little energy for planning." };
  }
  if (hasAny(text, ["family", "support others", "obligations", "household"])) {
    return { title: "Your income is carrying more than you.", body: "A full-time salary can look stable, but family or household obligations can quietly reduce the space for savings." };
  }
...
function getFreelanceSeasonSupportCopy(profile = {}) {
  const text = combineProfileText(profile);
  if (hasAny(text, ["irregular", "dry", "delayed", "client", "feast", "waves"])) {
    return { title: "Income timing is the real pressure.", body: "Freelance money can arrive in waves, so the challenge is keeping essentials safe between strong weeks and dry weeks." };
  }
  if (hasAny(text, ["overwork", "no rest", "too many projects", "client pressure", "rest feels risky"])) {
    return { title: "Freedom can still feel heavy.", body: "Flexible work can blur rest, earning, and pressure until the body carries what the schedule does not show." };
  }
  if (hasAny(text, ["personal/business", "separate", "work costs", "wallets"])) {
    return { title: "The money needs separation.", body: "When personal and work money mix, it becomes harder to see what is income, what is operating cost, and what is safe to spend." };
  }
...
function getBusinessBuilderSupportCopy(profile = {}) {
  const text = combineProfileText(profile);
  if (hasAny(text, ["reinvest", "growth", "scaling", "chase growth", "capital", "inventory"])) {
    return { title: "Growth is pulling on your money.", body: "Building a business often asks for reinvestment before profit feels stable, so personal stability can get squeezed if there is no boundary." };
  }
  if (hasAny(text, ["cash flow", "sales", "profit arrives late", "expenses happen before sales", "supplier", "customer"])) {
    return { title: "Cash flow needs protection.", body: "Sales and expenses rarely move perfectly together, so the pressure is often timing, not only profit." };
  }
  if (hasAny(text, ["personal/business", "mix", "pay myself", "owner pay", "personal life"])) {
    return { title: "Your money needs clearer lines.", body: "When business and personal money blend together, it becomes harder to know what belongs to growth and what protects your life." };
  }
...
function getCoreStageSupportCopy(stageKey, profile = {}) {
  switch (normalizeLifeStageKey(stageKey)) {
    case "Family Household":
      return getFamilyHouseholdSupportCopy(profile);
    case "Single Parent":
      return getSingleParentSupportCopy(profile);
    case "Full-Time Earner":
      return getFullTimeEarnerSupportCopy(profile);
    case "Freelance Season":
      return getFreelanceSeasonSupportCopy(profile);
    case "Business Builder":
      return getBusinessBuilderSupportCopy(profile);
    default:
      return null;
  }
}
...
function getCoreStageGuidanceCopy(stageKey) {
  switch (normalizeLifeStageKey(stageKey)) {
    case "Family Household":
      return { title: "Protect support with limits.", body: "Choose one boundary that lets you help without emptying your food, bills, or personal buffer." };
    case "Single Parent":
      return { title: "Protect the essentials first.", body: "Keep food, school, health, and transport visible before flexible spending gets any space." };
    case "Full-Time Earner":
      return { title: "Give payday a first move.", body: "Set one rule for salary day before rewards, subscriptions, or convenience spending begin." };
    case "Freelance Season":
      return { title: "Protect the dry-week gap.", body: "Keep a small buffer for essentials before project money turns into flexible spending." };
    case "Business Builder":
      return { title: "Separate growth from survival.", body: "Keep business costs, owner pay, and personal essentials from pulling from the same invisible pool." };
    default:
      return null;
  }
}
