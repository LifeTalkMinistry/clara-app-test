...
alter table if exists public.challenge_tasks
  add column if not exists tier_access jsonb default '["core_599","coaching_1299"]'::jsonb;
...
begin
  ...
  if tier_access_type = 'ARRAY' then
    ...
    execute $sql$
      ...
      alter column tier_access type jsonb
      using case
        when tier_access is null then '["core_599","coaching_1299"]'::jsonb
  ...
  execute $sql$
    alter table public.challenge_tasks
    alter column tier_access set default '["core_599","coaching_1299"]'::jsonb
  $sql$;
end;
$$;
...
create table if not exists public.user_programs (
  ...
  constraint user_programs_assigned_tier_check
    check (assigned_tier is null or assigned_tier in ('core_599', 'coaching_1299'))
);
...
normalized_seed as (
  select
    ...
    payload->>'short_label' as short_label,
    payload->>'theme' as theme,
    payload->>'description' as description,
    payload->>'why_this_matters' as why_this_matters,
    payload->>'task_instruction' as task_instruction,
    payload->>'reflection_prompt' as reflection_prompt,
    payload->>'journal_placeholder' as journal_placeholder,
    payload->>'question_1' as question_1,
    payload->>'question_2' as question_2,
    payload->>'question_3' as question_3,
    payload->>'completion_button_text' as completion_button_text,
    nullif(payload->>'milestone_type', '') as milestone_type,
    coalesce(nullif(payload->>'reward_title', ''), format('Day %s Complete', day)) as reward_title,
    payload->>'reward_message' as reward_message,
    (payload->>'estimated_minutes')::integer as estimated_minutes,
    coalesce(payload->'tier_access', '["core_599","coaching_1299"]'::jsonb) as tier_access,
    true as is_active,
    'active'::text as status,
    day as sort_order,
    day as day_number,
    ceil(day::numeric / 7)::integer as week_number,
    'reset_30'::text as program_family,
    format('day_%s', lpad(day::text, 2, '0')) as program_template_key,
    10 as main_points,
    10 as points,
    'none'::text as proof_required
  from seed
),
updated as (
  update public.challenge_tasks as target
  set
    title = source.title,
    ...
    journal_placeholder = source.journal_placeholder,
    question_1 = source.question_1,
    question_2 = source.question_2,
    question_3 = source.question_3,
    completion_button_text = source.completion_button_text,
    milestone_type = source.milestone_type,
    reward_title = source.reward_title,
    reward_message = source.reward_message,
    estimated_minutes = source.estimated_minutes,
    tier_access = source.tier_access,
    is_active = source.is_active,
    status = source.status,
    sort_order = source.sort_order,
    day = source.day,
    day_number = source.day_number,
    week = source.week_number,
    week_number = source.week_number,
    program_family = source.program_family,
    program_template_key = source.program_template_key,
    main_action_instruction = source.task_instruction,
    main_instruction = source.task_instruction,
    main_why_it_matters = source.why_this_matters,
    why_it_matters = source.why_this_matters,
    main_points = source.main_points,
    points = source.points,
    proof_required = source.proof_required,
    updated_at = timezone('utc'::text, now())
  from normalized_seed as source
  where target.program_template_key = source.program_template_key
  returning target.program_template_key
)
...
select
  ...
  source.journal_placeholder,
  source.question_1,
  source.question_2,
  source.question_3,
  source.completion_button_text,
  source.milestone_type,
  source.reward_title,
  source.reward_message,
  ...
  source.main_points,
  source.points,
  source.proof_required
from normalized_seed as source
left join updated on updated.program_template_key = source.program_template_key
where updated.program_template_key is null;
